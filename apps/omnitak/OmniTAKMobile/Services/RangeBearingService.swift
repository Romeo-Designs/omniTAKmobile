...
struct RangeBearingLine: Identifiable, Equatable {
    ...
    init(
        ...
        // Calculate grid convergence for accurate grid bearing
        let gridConvergence = Self.calculateGridConvergence(at: origin)
        var gridBear = bearingDegrees - gridConvergence
        gridBear = (gridBear + 360).truncatingRemainder(dividingBy: 360)
        self.gridBearing = gridBear
    }
    
    /// Calculate grid convergence at a given coordinate
    /// Grid convergence is the angle between true north and grid north
    private static func calculateGridConvergence(at coordinate: CLLocationCoordinate2D) -> Double {
        // Determine UTM zone
        let zone = Int((coordinate.longitude + 180.0) / 6.0) + 1
        
        // Calculate central meridian for the zone
        let centralMeridian = Double(zone - 1) * 6.0 - 180.0 + 3.0
        
        // Grid convergence formula: γ = (λ - λ₀) × sin(φ)
        // where λ = longitude, λ₀ = central meridian, φ = latitude
        let deltaLon = coordinate.longitude - centralMeridian
        let convergence = deltaLon * sin(coordinate.latitude * .pi / 180.0)
