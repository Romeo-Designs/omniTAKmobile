...
class NavigationService: NSObject, ObservableObject {
    ...
    func startNavigation(to waypoint: Waypoint) {
        ...
        updateNavigationCalculations()
        
        print("🧭 Started navigation to \(waypoint.name)")
    }
    
    /// Update navigation calculations (distance, bearing, ETA) for current target
    private func updateNavigationCalculations() {
        guard navigationState.isNavigating,
              let targetWaypoint = navigationState.targetWaypoint,
        ...
        // Update distance
        navigationState.currentDistance = currentLoc.distance(from: targetLocation)
    ...
    /// Calculate optimal route through multiple waypoints using great circle distances
    func optimizeRoute(_ waypoints: [Waypoint]) -> [Waypoint] {
        guard waypoints.count > 2 else { return waypoints }
        
        // Simple nearest-neighbor optimization (TSP approximation)
        var optimized: [Waypoint] = []
        ...
        while !remaining.isEmpty {
            ...
            optimized.append(remaining[nearestIndex])
            remaining.remove(at: nearestIndex)
        }
        
        let totalDistance = calculateTotalDistance(optimized)
        print("🗺️ Route optimized: \(waypoints.count) waypoints, \(Int(totalDistance))m total")
        
        return optimized
    }
    
    /// Calculate total distance along route
    private func calculateTotalDistance(_ waypoints: [Waypoint]) -> Double {
        var total: Double = 0
    ...
    func calculateETA() -> Date? {
        guard let location = currentLocation,
              let waypoint = navigationState.targetWaypoint,
              let avgSpeed = navigationState.averageSpeed,
              avgSpeed > 0 else {
            return nil
        }
        
        let waypointLoc = CLLocation(
            latitude: waypoint.coordinate.latitude,
            longitude: waypoint.coordinate.longitude
        )
        
        let distance = location.distance(from: waypointLoc)
        let timeRemaining = distance / avgSpeed
