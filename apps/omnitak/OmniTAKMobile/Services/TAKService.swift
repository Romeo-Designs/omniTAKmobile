...
class TAKService: ObservableObject {
    ...
    init() {
        print("🔧 TAKService.init() - Creating new TAKService instance: \(ObjectIdentifier(self))")
        
        // Initialize the omnitak library
        // NOTE: On macOS, omnitak_init() may log battery status errors from the Rust FFI layer.
        // These errors are harmless and can be ignored - they occur because the Rust code
        // attempts to query battery status using iOS APIs that don't exist on macOS.
        // The Swift layer already has proper platform guards in PositionBroadcastService.
        ...
        print("🔧 TAKService.init() - Initialization complete for instance: \(ObjectIdentifier(self))")
    }
    ...
    private func handleReceivedMessage(_ xml: String) {
        ...
        // Note: nil return from parser means message was intentionally ignored (e.g., t-x-takp-v)
        // This is not an error, so we don't log a warning
    }
    ...
    func sendCoT(xml: String) -> Bool {
        ...
        if directTCP.send(xml: xml) {
            ...
            // Also send via Rust FFI (fully functional, provides additional protocol support)
            // The Rust implementation handles TCP/TLS/UDP/Meshtastic with proper message framing
            if connectionHandle > 0 {
                let xmlCStr = xml.cString(using: .utf8)!
                let result = omnitak_send_cot(connectionHandle, xmlCStr)
                if result != 0 {
                    #if DEBUG
                    print("⚠️ Rust FFI send failed with code: \(result)")
                    #endif
                }
            }
