...
class PositionBroadcastService: ObservableObject {
    ...
    func startBroadcasting() {
        guard takService != nil, locationManager != nil else {
            let missingServices = [
                takService == nil ? "TAKService" : nil,
                locationManager == nil ? "LocationManager" : nil
            ].compactMap { $0 }.joined(separator: ", ")
            
            print("Cannot start broadcasting: \(missingServices) not configured")
            lastError = "Services not configured: \(missingServices)"
            return
        }
    ...
    private func generateSelfSACoT(location: CLLocation) -> String {
        ...
        // Use configured unit type
        let cotType = userUnitType
    ...
    private func startBatteryMonitoring() {
        ...
        #elseif os(macOS)
        // macOS doesn't have UIDevice battery monitoring
        // Set default values for desktop systems
        batteryLevel = 1.0
        deviceStatus = "Operational"
        print("Battery monitoring not available on macOS - using default values")
        #endif
    }
    ...
    private func updateBatteryLevel() {
        ...
        if level >= 0 {
            ...
            // Handle case where battery level is unavailable (-1.0)
            print("Battery level unavailable, using default value")
            batteryLevel = 1.0
        }
        ...
        #elseif os(macOS)
        // macOS doesn't support battery monitoring via UIDevice
        batteryLevel = 1.0
        deviceStatus = "Operational"
        #endif
    }
    ...
    private func saveBroadcastSettings() {
        ...
        UserDefaults.standard.set(userUnitType, forKey: "userUnitType")
    }
    ...
    private func loadBroadcastSettings() {
        ...
        if let savedUnitType = UserDefaults.standard.string(forKey: "userUnitType") {
            print("📥 Loading saved unit type: \(savedUnitType)")
            userUnitType = savedUnitType
        } else {
            print("📥 No saved unit type, using default: \(userUnitType)")
        }
    }
