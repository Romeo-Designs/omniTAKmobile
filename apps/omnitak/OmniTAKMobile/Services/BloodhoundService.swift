...
struct BloodhoundTrack: Identifiable, Codable {
    ...
    // Predict future position with enhanced algorithm
    func predictPosition(secondsAhead: TimeInterval) -> CLLocationCoordinate2D? {
        guard let current = currentPosition else {
            return nil
        }
        
        // Enhanced prediction using velocity history
        if positions.count >= 3 {
            // Use last 3 positions for acceleration-aware prediction
            let p1 = positions[positions.count - 3]
            let p2 = positions[positions.count - 2]
            let p3 = positions[positions.count - 1]
            
            // Calculate velocity change (acceleration)
            let v1 = calculateVelocityVector(from: p1, to: p2)
            let v2 = calculateVelocityVector(from: p2, to: p3)
            let acceleration = ((v2.0 - v1.0), (v2.1 - v1.1))
            
            // Project with acceleration
            let projectedLat = p3.coordinate.latitude + v2.0 * secondsAhead + 0.5 * acceleration.0 * secondsAhead * secondsAhead
            let projectedLon = p3.coordinate.longitude + v2.1 * secondsAhead + 0.5 * acceleration.1 * secondsAhead * secondsAhead
            
            return CLLocationCoordinate2D(latitude: projectedLat, longitude: projectedLon)
        }
        
        // Fallback to simple linear projection
        guard let speed = currentSpeed,
    ...
    /// Calculate velocity vector between two positions (degrees per second)
    private func calculateVelocityVector(from: TrackPosition, to: TrackPosition) -> (Double, Double) {
        let deltaTime = to.timestamp.timeIntervalSince(from.timestamp)
        ...
        return (deltaLat / deltaTime, deltaLon / deltaTime)
    }
}
...
class BloodhoundService: ObservableObject {
    ...
    func getLocationHistory(for uid: String, from startDate: Date, to endDate: Date) -> [TrackPosition] {
        guard let track = tracks[uid] else { return [] }
        
        return track.positions.filter { position in
            position.timestamp >= startDate && position.timestamp <= endDate
        }
    }
    
    /// Check proximity to other tracks and return alerts
    func checkProximityAlerts(for uid: String, withinMeters threshold: Double) -> [ProximityAlert] {
        guard let track = tracks[uid],
              let currentPos = track.currentPosition else { return [] }
        
        var alerts: [ProximityAlert] = []
        let currentLoc = CLLocation(
            latitude: currentPos.coordinate.latitude,
            longitude: currentPos.coordinate.longitude
        )
        
        for (otherUid, otherTrack) in tracks {
            guard otherUid != uid,
                  let otherPos = otherTrack.currentPosition,
                  otherTrack.isOnline else { continue }
            
            let otherLoc = CLLocation(
                latitude: otherPos.coordinate.latitude,
                longitude: otherPos.coordinate.longitude
            )
            ...
            if distance <= threshold {
                alerts.append(ProximityAlert(
                    targetCallsign: otherTrack.callsign,
                    targetUid: otherUid,
                    distance: distance,
                    bearing: currentLoc.bearing(to: otherLoc)
                ))
            }
        }
        
        return alerts.sorted { $0.distance < $1.distance }
    }
...
struct ProximityAlert: Identifiable {
    let id = UUID()
