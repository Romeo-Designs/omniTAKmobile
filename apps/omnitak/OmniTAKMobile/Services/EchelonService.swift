...
class EchelonService: ObservableObject {
    ...
    func getAllSubordinates(of unitId: UUID) -> [MilitaryUnit] {
        guard let unit = units[unitId] else { return [] }
        
        var subordinates: [MilitaryUnit] = []
        
        for childId in unit.childrenIds {
            if let child = units[childId] {
                subordinates.append(child)
                subordinates.append(contentsOf: getAllSubordinates(of: childId))
            }
        }
    ...
    /// Get the commander of a unit (parent unit)
    func getCommander(of unitId: UUID) -> MilitaryUnit? {
        guard let unit = units[unitId],
              let parentId = unit.parentId else { return nil }
        return units[parentId]
    }
    
    /// Calculate unit strength (aggregate from subordinates)
    func calculateUnitStrength(unitId: UUID) -> UnitStrength {
        guard let unit = units[unitId] else {
            return UnitStrength(personnel: 0, equipment: 0, readiness: 0)
        }
        
        var totalPersonnel = unit.currentStrength
        var equipmentCount = 0
        var readinessSum = unit.status.effectivenessMultiplier * 100
        var unitCount = 1
        
        // Aggregate from subordinates
        let subordinates = getAllSubordinates(of: unitId)
        for sub in subordinates {
            totalPersonnel += sub.currentStrength
            readinessSum += sub.status.effectivenessMultiplier * 100
            unitCount += 1
        }
        ...
        return UnitStrength(
            ...
            readiness: readinessSum / Double(unitCount)
        )
    }
    
    /// Aggregate status from subordinate units
    func aggregateStatus(for unitId: UUID) -> UnitStatus {
        guard let unit = units[unitId] else { return .operational }
        ...
        // If any subordinate is combat ineffective, escalate
        if subordinates.contains(where: { $0.status == .combatIneffective }) {
            return .combatIneffective
        }
        
        // If majority degraded, roll up as degraded
        let degradedCount = subordinates.filter { $0.status == .degraded }.count
        if Double(degradedCount) / Double(subordinates.count) > 0.5 {
            return .degraded
        }
        
        // If all operational, report operational
        if subordinates.allSatisfy({ $0.status == .operational }) {
            return .operational
        }
