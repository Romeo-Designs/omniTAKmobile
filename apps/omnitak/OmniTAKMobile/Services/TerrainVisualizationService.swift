...
class TerrainVisualizationService: NSObject, ObservableObject {
    ...
    @Published var terrainExaggeration: TerrainExaggeration = .none {
        didSet {
            UserDefaults.standard.set(terrainExaggeration.rawValue, forKey: "terrainExaggeration")
        }
    }
    ...
    @Published var showElevationAwareness: Bool = true {
        didSet {
            UserDefaults.standard.set(showElevationAwareness, forKey: "showElevationAwareness")
        }
    }
    ...
    func getTerrainElevation(at coordinate: CLLocationCoordinate2D) async -> Double {
        // Try real elevation API first
        do {
            let elevation = try await ElevationAPIClient.shared.getElevation(for: coordinate)
            return max(0, elevation)
        } catch {
            // Fallback to simulation when offline or API fails
            print("⚠️ Elevation API failed for terrain visualization, using simulation: \(error.localizedDescription)")
            return simulateTerrainElevation(at: coordinate)
        }
    }
    
    private func simulateTerrainElevation(at coordinate: CLLocationCoordinate2D) -> Double {
        // Multi-frequency terrain simulation for offline fallback
