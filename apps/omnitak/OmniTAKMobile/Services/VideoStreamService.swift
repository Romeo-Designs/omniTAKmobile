...
class VideoStreamService: ObservableObject {
    ...
    func addFeed(_ feed: VideoFeed) {
        ...
        guard validateStreamProtocol(feed.url) else {
            print("VideoStreamService: Unsupported stream protocol for feed \(feed.name)")
            return
        }
    ...
    /// Validate stream protocol support (RTSP, HLS, HTTP)
    private func validateStreamProtocol(_ url: String) -> Bool {
        let lowercased = url.lowercased()
        return lowercased.hasPrefix("rtsp://") ||
    ...
    func testStream(_ feed: VideoFeed, completion: @escaping (Bool, String?) -> Void) {
        guard let url = URL(string: feed.url) else {
            completion(false, "Invalid URL format")
            return
        }
        
        if feed.url.lowercased().hasPrefix("rtsp://") {
            // RTSP streams require AVPlayer validation
            completion(true, "RTSP stream ready for playback")
        } else {
            // HTTP/HLS validation
            let task = URLSession.shared.dataTask(with: url) { _, response, error in
                ...
                if let httpResponse = response as? HTTPURLResponse {
                    completion(httpResponse.statusCode == 200, "Stream reachable")
                } else {
                    completion(true, "Stream available")
                }
            }
            task.resume()
        }
