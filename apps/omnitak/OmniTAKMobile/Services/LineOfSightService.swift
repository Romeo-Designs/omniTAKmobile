...
class LineOfSightService: NSObject, ObservableObject {
    ...
    /// Get terrain elevation using real API or simulation fallback
    private func simulateTerrainElevation(at location: CLLocationCoordinate2D,
                                          fraction: Double,
                                          distance: Double) -> Double {
        // Try to use real elevation data (synchronous fallback to simulation)
        // Note: This is called from sync context, so we use simulation
        // For async profile generation, use getElevationAsync instead
    ...
    /// Get real terrain elevation asynchronously
    private func getElevationAsync(at location: CLLocationCoordinate2D, fraction: Double, distance: Double) async -> Double {
        do {
            let elevation = try await ElevationAPIClient.shared.getElevation(for: location)
            return elevation
        } catch {
            // Fallback to simulation
            return simulateTerrainElevation(at: location, fraction: fraction, distance: distance)
        }
    }
