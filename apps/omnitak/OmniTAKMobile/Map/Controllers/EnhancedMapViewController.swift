...
class EnhancedMapViewController: UIViewController {
    ...
    // Marker tracking
    private var isTrackingMarker = false
    private var trackedMarker: EnhancedCoTMarker?
    ...
    private func messageMarker(_ marker: EnhancedCoTMarker) {
        // Open chat conversation with marker's contact
        let chatManager = ChatManager.shared
        
        // Create or get existing conversation
        let conversationId = chatManager.getOrCreateConversation(
            withUID: marker.uid,
        ...
        NotificationCenter.default.post(
            ...
            object: nil,
            userInfo: ["conversationId": conversationId, "callsign": marker.callsign]
        )
        
        print("📱 Opening chat with: \(marker.callsign)")
    }
    ...
    private func trackMarker(_ marker: EnhancedCoTMarker) {
        ...
        // Enable trails for tracked marker
        showTrails = true
        updateTrail(for: marker)
        
        // Set up auto-follow with periodic updates
        startTrackingUpdates(for: marker)
        
        // Show tracking indicator in UI
        NotificationCenter.default.post(
            name: NSNotification.Name("MarkerTrackingEnabled"),
            object: nil,
            userInfo: ["markerUID": marker.uid, "callsign": marker.callsign]
        )
    }
    
    private func startTrackingUpdates(for marker: EnhancedCoTMarker) {
        // Stop any existing tracking
        stopTrackingUpdates()
        
        // Create timer to update map center on tracked marker
        trackingUpdateTimer = Timer.scheduledTimer(withTimeInterval: 2.0, repeats: true) { [weak self] _ in
            guard let self = self,
                  let tracked = self.trackedMarker,
                  self.isTrackingMarker else { return }
            
            // Update map center to follow marker
            let region = MKCoordinateRegion(
                center: tracked.coordinate,
                span: self.mapView.region.span // Keep current zoom level
            )
            self.mapView.setRegion(region, animated: true)
            
            // Update trail
            self.updateTrail(for: tracked)
        }
    }
    ...
    func stopTrackingUpdates() {
        ...
        NotificationCenter.default.post(
            name: NSNotification.Name("MarkerTrackingDisabled"),
            object: nil
        )
    }
