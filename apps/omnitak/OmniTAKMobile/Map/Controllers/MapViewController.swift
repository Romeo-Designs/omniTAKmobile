...
struct ATAKMapView: View {
    ...
    @State private var mapRegion = MKCoordinateRegion(
        center: CLLocationCoordinate2D(latitude: 36.1600, longitude: -86.8005), // Default: Nashville, TN
    ...
    // Marker info panel
    @State private var showMarkerInfoPanel = false
    @State private var selectedMarker: EnhancedCoTMarker?

    // Position broadcasting service
    @ObservedObject private var positionBroadcastService = PositionBroadcastService.shared
    
    // Emergency beacon location manager
    @StateObject private var beaconLocationManager = BeaconLocationManager()

    // Layer states
    @State private var activeMapLayer = "standard"
    @State private var showFriendly = true
    @State private var showHostile = true
    @State private var showUnknown = true

    // Map overlay states
    @State private var showCompass = true  // Hidden by default for max map space
    ...
    // Using enhancedMarkers for better state management (prevents flickering)
    private var cotMarkers: [EnhancedCoTMarker] {
        takService.enhancedMarkers.values.filter { marker in
            // Filter based on overlay settings
            if marker.affiliation == .friendly && !showFriendly {
                return false
            }
            if marker.affiliation == .hostile && !showHostile {
                return false
            }
            if marker.affiliation == .unknown && !showUnknown {
                return false
            }
            return true
        }
    ...
    private var mainMapView: some View {
        TacticalMapView(
            ...
            showMarkerInfoPanel: $showMarkerInfoPanel
        )
        .ignoresSafeArea()
    }
    ...
    @ViewBuilder
    private var markerInfoPanelOverlay: some View {
        if showMarkerInfoPanel, let marker = selectedMarker {
            ZStack {
            ...
            .zIndex(2000) // Very high z-index to be above everything
            .transition(.opacity)
        }
    }
    ...
    private var lifecycleHandlers: some View {
        EmptyView()
            .onAppear {
                ...
                // Configure EmergencyBeaconService with TAKService and location
                EmergencyBeaconService.shared.configure(
                    takService: takService,
                    locationManager: beaconLocationManager,
                    callsign: positionBroadcastService.userCallsign
                )
                
                // Register callback for incoming chat messages
                takService.onChatMessageReceived = { chatMessage in
                    chatManager.receiveMessage(chatMessage)
                }
    ...
    private func toggleLayer(_ layer: String) {
        // Safe haptic feedback
        provideSafeHapticFeedback(style: .light)
    ...
    // MARK: - Marker Interaction Methods
    
    func showMarkerInfoPanel(for marker: EnhancedCoTMarker) {
        selectedMarker = marker
        showMarkerInfoPanel = true
    }
    
    private func sendChatToMarker(_ marker: EnhancedCoTMarker) {
        // Close info panel
        showMarkerInfoPanel = false
        
        // Open chat with pre-filled recipient
        chatManager.startDirectChat(with: marker.uid, callsign: marker.callsign)
        showChat = true
    }
    ...
    private func trackMarker(_ marker: EnhancedCoTMarker) {
        ...
        // Start tracking this marker (will follow its position updates)
        // This would integrate with a tracking service
        print("📍 Tracking marker: \(marker.callsign)")
    ...
    private func provideSafeHapticFeedback(style: UIImpactFeedbackGenerator.FeedbackStyle) {
        #if !targetEnvironment(macCatalyst)
        DispatchQueue.main.async {
...
// MARK: - Enhanced CoT Marker Annotation

/// Custom annotation class that wraps EnhancedCoTMarker for proper state management
class EnhancedCoTMarkerAnnotation: NSObject, MKAnnotation {
    var marker: EnhancedCoTMarker
    
    dynamic var coordinate: CLLocationCoordinate2D {
        get { marker.coordinate }
        set { 
            // Note: coordinate is read-only in EnhancedCoTMarker
            // Updates should be done by replacing the entire marker
...
struct TacticalMapView: UIViewRepresentable {
    ...
    let markers: [EnhancedCoTMarker]  // Changed from CoTMarker to EnhancedCoTMarker
    ...
    // Marker selection bindings
    @Binding var selectedMarker: EnhancedCoTMarker?
    @Binding var showMarkerInfoPanel: Bool
    ...
    private func updateAnnotations(mapView: MKMapView, markers: [EnhancedCoTMarker], context: Context) {
        // Get existing CoT marker annotations
        let existingMarkerAnnotations = mapView.annotations.compactMap { $0 as? EnhancedCoTMarkerAnnotation }
        let existingUIDs = Set(existingMarkerAnnotations.map { $0.marker.uid })
        ...
        let annotationsToRemove = existingMarkerAnnotations.filter { toRemove.contains($0.marker.uid) }
        mapView.removeAnnotations(annotationsToRemove)
        
        // Update existing or add new annotations
        for marker in markers {
            if let existingAnnotation = existingMarkerAnnotations.first(where: { $0.marker.uid == marker.uid }) {
                // Update existing annotation if marker data changed
                if existingAnnotation.marker.coordinate.latitude != marker.coordinate.latitude ||
                   existingAnnotation.marker.coordinate.longitude != marker.coordinate.longitude ||
                   existingAnnotation.marker.callsign != marker.callsign {
                    existingAnnotation.marker = marker
                    existingAnnotation.coordinate = marker.coordinate
                    
                    // Force view update by removing and re-adding
                    if let view = mapView.view(for: existingAnnotation) as? MilStd2525MapAnnotationView {
                        view.configure(cotType: marker.type, callsign: marker.callsign, echelon: nil)
                    }
                }
            } else {
                // Add new annotation
                let annotation = EnhancedCoTMarkerAnnotation(marker: marker)
    ...
    class Coordinator: NSObject, MKMapViewDelegate {
        ...
        func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
            ...
            // Handle EnhancedCoTMarkerAnnotation with MIL-STD-2525 symbols
            if let markerAnnotation = annotation as? EnhancedCoTMarkerAnnotation {
                let identifier = "CoTMarker_MilStd_\(markerAnnotation.marker.uid)"
                var annotationView = mapView.dequeueReusableAnnotationView(withIdentifier: identifier) as? MilStd2525MapAnnotationView

                if annotationView == nil {
                    print("🆕 Creating marker for \(markerAnnotation.marker.callsign)")
                ...
                // Configure with EnhancedCoTMarker data
                print("⚙️ Configuring \(markerAnnotation.marker.callsign): \(markerAnnotation.marker.type)")
            ...
            // Fallback for legacy MKPointAnnotation (should not normally hit this)
        ...
        func mapView(_ mapView: MKMapView, didSelect view: MKAnnotationView) {
            print("📍 Annotation selected: \(type(of: view.annotation))")
            
            // Handle CoT marker selection
            if let markerAnnotation = view.annotation as? EnhancedCoTMarkerAnnotation {
                let marker = markerAnnotation.marker
                print("✅ CoT marker selected: \(marker.callsign)")
                
                // Update view to show selection
                if let milStdView = view as? MilStd2525MapAnnotationView {
                    milStdView.isMarkerSelected = true
                    print("✅ Set marker as selected")
                }
                
                // Show marker info panel with actions
                DispatchQueue.main.async { [weak self] in
                    guard let self = self else { return }
                    print("✅ Setting showMarkerInfoPanel = true")
                    self.parent.selectedMarker = marker
                    self.parent.showMarkerInfoPanel = true
                }
            } else {
                print("❌ Not an EnhancedCoTMarkerAnnotation: \(String(describing: view.annotation))")
            }
        }
        ...
        func mapView(_ mapView: MKMapView, didDeselect view: MKAnnotationView) {
            // Handle deselection
            if let milStdView = view as? MilStd2525MapAnnotationView {
                milStdView.isMarkerSelected = false
            }
        }
