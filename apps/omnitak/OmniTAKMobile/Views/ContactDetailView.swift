...
struct ContactDetailView: View {
        ...
        NavigationView {
            ...
            .navigationBarItems(
                ...
                trailing: Menu {
                    ...
                    Divider()
                    Button(role: .destructive, action: { blockContact() }) {
    ...
    private func showOnMap() {
        // Post notification to main map view to center on contact
        // The map controller will look up the marker position from TAKService
        NotificationCenter.default.post(
            name: NSNotification.Name("ShowContactOnMap"),
            object: nil,
            userInfo: [
    ...
    private func navigateToContact() {
        // Post notification to start navigation to contact
        // The map controller will look up the marker position from TAKService
        NotificationCenter.default.post(
            name: NSNotification.Name("NavigateToContact"),
            object: nil,
            userInfo: [
                "contactUID": contact.id,
                "callsign": contact.callsign
    ...
    private func shareContact() {
        // Create vCard or CoT XML for sharing
        let contactInfo = """
        Callsign: \(contact.callsign)
        UID: \(contact.id)
        Endpoint: \(contact.endpoint ?? "Unknown")
        Status: \(contact.isOnline ? "Online" : "Offline")
        """
        
        // Use UIActivityViewController for sharing
        let activityVC = UIActivityViewController(
            activityItems: [contactInfo],
    ...
    private func blockContact() {
        // Add contact to blocked list in ChatManager
        chatManager.blockContact(contact.id)
        dismiss()
    }
