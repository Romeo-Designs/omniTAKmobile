...
struct CertificateEnrollmentResponse: Codable {
    let certificate: Foundation.Data  // .p12 certificate data
    let password: String   // Certificate password
    let serverHost: String
    let serverPort: Int
    let serverProtocol: String
    let caCertificate: Foundation.Data?  // Optional CA certificate
}
...
class CertificateManager: ObservableObject {
    ...
    func loadCertificate(id: UUID) throws -> (p12Data: Foundation.Data, password: String) {
    ...
    private func saveToKeychain(key: String, data: Foundation.Data) throws {
    ...
    private func loadFromKeychain(key: String) -> Foundation.Data? {
        let query: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrService as String: keychainService,
            kSecAttrAccount as String: key,
    ...
    private func extractIdentity(from p12Data: Foundation.Data, password: String) throws -> SecIdentity? {
    ...
    private func getCertificateExpiry(_ certificate: SecCertificate) -> Date? {
        ...
        guard let data = SecCertificateCopyData(certificate) as Foundation.Data? else {
            return nil
        }
    ...
    private func parseDERLength(_ data: Foundation.Data, offset: Int) -> (length: Int, size: Int) {
