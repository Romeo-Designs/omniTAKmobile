...
class DataPackageManager: ObservableObject {
    ...
    func importPackage(from url: URL) async throws -> PackageImportResult {
        ...
        let conflict = detectPackageConflict(package)
        if let existingPackage = conflict {
            print("⚠️ Package conflict detected with existing package: \(existingPackage.name)")
            // Resolve conflict - keep newer version
            let resolution = resolvePackageConflict(new: package, existing: existingPackage)
            
            await MainActor.run {
                if let indexToReplace = packages.firstIndex(where: { $0.id == existingPackage.id }) {
                    packages[indexToReplace] = resolution
                    print("✅ Replaced package with newer version")
                } else {
                    packages.append(resolution)
                }
            }
        } else {
            // No conflict, add new package
            await MainActor.run {
    ...
    private func detectPackageConflict(_ newPackage: DataPackage) -> DataPackage? {
        // Check for packages with same name
        return packages.first { existing in
            existing.name == newPackage.name
        }
    }
    
    private func resolvePackageConflict(new: DataPackage, existing: DataPackage) -> DataPackage {
        // Compare versions and keep newer
        let newVersion = parseVersion(new.version)
        let existingVersion = parseVersion(existing.version)
        
        let comparison = compareVersions(newVersion, existingVersion)
        
        if comparison > 0 {
            print("✅ Keeping newer package version: \(new.version) > \(existing.version)")
            return new
        } else if comparison == 0 {
            // Same version - check checksums
            if new.checksum != existing.checksum {
                print("⚠️ Same version but different checksums - keeping newer by date")
                return new.importDate > existing.importDate ? new : existing
            }
            return existing
        } else {
            print("ℹ️ Keeping existing package version: \(existing.version) > \(new.version)")
            return existing
        }
    }
    ...
    private var metadataCache: [UUID: PackageMetadataCache] = [:]
    
    private func cachePackageMetadata(_ package: DataPackage) {
        let cache = PackageMetadataCache(
            packageId: package.id,
            name: package.name,
            version: package.version,
            contentCount: package.contents.count,
            size: package.size,
            overlayCount: package.overlayCount,
            iconCount: package.iconCount,
            lastAccessed: Date()
        )
        metadataCache[package.id] = cache
        saveMetadataCache()
    }
    
    private func saveMetadataCache() {
        let cacheFile = fileManager.urls(for: .cachesDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("package_metadata_cache.json")
        
        do {
            let encoder = JSONEncoder()
            let data = try encoder.encode(Array(metadataCache.values))
    ...
    private func loadMetadataCache() {
        ...
        do {
            ...
            let caches = try JSONDecoder().decode([PackageMetadataCache].self, from: data)
            metadataCache = Dictionary(uniqueKeysWithValues: caches.map { ($0.packageId, $0) })
        } catch {
