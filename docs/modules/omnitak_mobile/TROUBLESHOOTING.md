...
```
ERROR: .../docs/modules/omnitak_mobile/BUILD.bazel:23:10: no such target '//modules/omnitak_mobile:ios/native/OmniTAKMobile.xcframework/ios-arm64/libomnitak_mobile.a'
```
...
```bash
ls -la $PROJECT_ROOT/docs/modules/omnitak_mobile/ios/native/OmniTAKMobile.xcframework/
```
...
cp target/aarch64-linux-android/release/libomnitak_mobile.so \
   $PROJECT_ROOT/docs/modules/omnitak_mobile/android/jniLibs/arm64-v8a/
```
...
kt_android_library(
    name = "android_maplibre_wrapper",
    proguard_specs = ["//docs/modules/omnitak_mobile/android:proguard_rules"],
)
```
