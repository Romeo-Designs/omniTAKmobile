                ...
                cmd = rx.recv() => {
                    match cmd {
                        Some(ClientCommand::Send(xml)) => {
                            // Add newline termination for TAK protocol
                            let message = if xml.ends_with('\n') {
                                xml
                            } else {
                                format!("{}\n", xml)
                            };
                            ...
                            if let Err(e) = write_half.write_all(message.as_bytes()).await {
                                ...
                                debug!("Sent CoT message ({} bytes)", message.len());
                            }
                        }
                    ...
                    match cmd {
                        Some(ClientCommand::Send(xml)) => {
                            ...
                            if let Err(e) = stream.write_all(message.as_bytes()).await {
                                ...
                                debug!("Sent CoT message via TLS ({} bytes)", message.len());
                            }
                        }
                        Some(ClientCommand::Disconnect) | None => {
                    ...
                    match cmd {
                        Some(ClientCommand::Send(xml)) => {
                            ...
                            if let Err(e) = socket.send(message.as_bytes()).await {
                                ...
                                debug!("Sent CoT message via UDP ({} bytes)", message.len());
                            }
                        }
                        Some(ClientCommand::Disconnect) | None => {
...
mod tests {
    ...
    fn test_message_framing() {
        // Test that messages get newline termination
        let xml_without_newline = "<?xml version=\"1.0\"?><event></event>";
        let xml_with_newline = "<?xml version=\"1.0\"?><event></event>\n";

        // Simulate the framing logic
        let message1 = if xml_without_newline.ends_with('\n') {
            xml_without_newline.to_string()
        } else {
            format!("{}\n", xml_without_newline)
        };
        assert!(message1.ends_with('\n'), "Message should have newline termination");

        let message2 = if xml_with_newline.ends_with('\n') {
            xml_with_newline.to_string()
        } else {
            format!("{}\n", xml_with_newline)
        };
        assert!(message2.ends_with('\n'), "Message should preserve existing newline");
        assert_eq!(message2.matches('\n').count(), 1, "Should not double newlines");
    }

    #[test]
    fn test_send_cot_state_update() {
        // Test that sending updates the message count
        // Note: This would require mocking or refactoring to test properly
        // For now, just verify the API exists
        let config = ConnectionConfig::new("localhost", 8087, Protocol::Tcp);
        // The send_cot method exists and has the right signature
        let _ = TakClient::connect(config, None);
    }
}
